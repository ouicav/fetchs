...
- 安全和稳定性：`fetchs`单纯由本人借鉴axios的请求行为加上自己想法和思路创造出来，安全和稳定性未知，仅供学习交流使用。
- 实现原理机制：发起请求时`fetchs`会监听调用`fetchs()`时的实参(请求配置对象)，在`fetchs`中，接收这些参数，每个参数都有一个不同的函数去处理，当参数处理完成后，才会生成一个`Request`实列对象，`fetch`执行时，传入这个`Request`实列对象，真正的发起请求。请求任务完成后，在`then()`中接收一个`Response` 对象，这个对象描述了响应回来的数据，经过一些处理后`Response` 对象中响应数据体最终成为`Promise` 实列对象，并抛出由`fetchs()`返回。
- 一般的强烈建议使用 `try/catch` 与 `async/await` 配合 `fetchs` 使用。
...
- **单独的请求的配置对象**
  ...
  1. **注意**：`url参数`是必填的，其他`参数`可以根据接口要求去配置，可有可无。
  2. **注意**：如果`body参数`的值是一个`object`数据类型，那么发送请求时`fetchs`会自动把这个对象转化为`json数据格式`发送。
  3. **注意**：发送`body参数`的请求体时，浏览器会根据请求体的数据类型，自动配置相应的请求头(headers)。
  4. **注意**：设置`time参数`后，发送请求时，如果服务器没有在设定的时间内有响应，那么`fetchs`会自动取消这次请求，并抛出错误信息由`try/catch`语句中的`catch块`的错误对象`err`接收。
  5. **注意**：参数顺序没有固定。
  6. **注意**：如果不配置`format参数`，当请求成功响应后，那么`fetchs`会自动把成功响应回来的数据当作`json格式`以反序列化的形式转化数据，如果转化失败，那么会把响应数据当作xml ，srting，json等字符格式的数据类型转化成text数据格式, 如果还是转化失败，则抛出错误。
  7. **注意**：`query参数`，`time参数`，`format参数`，`progress参数 `这四个参数功能都是独立封装的，并不在`Request`实列对象的支持中
  8. **参数配置**
...
- **统一参数请求的配置对象**
  ...
  2. **注意**： 除了`baseUrl参数`与**单独的请求的配置对象**的`url参数`不一样，其他`参数`功能都是一样。

  3. **注意**：如果**统一参数请求的配置对象**`参数`与**绑定的单独请求配置对象**`参数`一样，但是值不一样，则**绑定的单独请求配置对象**`参数`优先生效，请看下面的代码。
...
- 在这第二个函数中，每次接收到`Uin8Array数据块`就会自动触发这个函数，传递三个实参，分别是：`Uin8Array数据块`、  `Uin8Array数据块大小`  、当前传输的百分比进度。

- **这里要注意的是**：想要实现下载进度，必须确保`Response`实列对象中的`headers`有`Content-Length`且值不为0, 否则 传输的百分比进度参数值为`null`。
...
- `fetchs`默认遵循CORS协议的跨域请求

- `get`请求不能携带`body`请求体

- 正常的响应成功后也就是响应状态码在200~299之间的会在`try`中抛出格式化后的`响应数据体`

- 正常的失败后，也就是服务器接收到了，有响应但是状态码为200~299以外的会在`catch`抛出失败的响应数据。
